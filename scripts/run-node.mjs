...
const compilerOverride = env.CLUSTERCLAW_TS_COMPILER ?? env.OPENCLAW_TS_COMPILER ?? env.CLAWDBOT_TS_COMPILER;
...
const shouldBuild = () => {
  if (env.CLUSTERCLAW_FORCE_BUILD === "1" || env.OPENCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.CLUSTERCLAW_RUNNER_LOG === "0" || env.OPENCLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[clusterclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["clusterclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
